...
const ProductDetailView = () => {
    ...
    // --- HELPER LOGIC: Combo ki details combine karne ke liye ---
    // Agar Combo ka apna 'how_to_use' khali hai, toh included products ka mix karke dikhao
    const getCombinedUsage = () => {
        ...
        // Agar main combo view hai aur details missing hain
    ...
    const getCombinedFeatures = () => {
        ...
        // Agar Combo view mein features nahi hain, toh items ke features uthao
        if (activeDisplay?.is_combo && activeDisplay.included_products) {
            let allFeats = [];
            activeDisplay.included_products.forEach(p => {
                if (p.features) allFeats = [...allFeats, ...p.features];
            });
            return allFeats.slice(0, 6); // Top 6 features dikhao
        }
        return [];
    };

    if (loading) return <div className="min-h-screen flex items-center justify-center bg-white font-black text-emerald-500 animate-pulse uppercase tracking-widest">Loading {productId}...</div>;
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-0">
                        <div className="p-10 border-r border-slate-50 bg-white">
                            <div className="relative aspect-square bg-slate-50 rounded-[3rem] overflow-hidden mb-6 group">
                                <img src={mainImage} className="w-full h-full object-contain p-12 group-hover:scale-105 transition-all duration-700" alt="product" />
                            </div>
                            <div className="flex gap-4 overflow-x-auto pb-4 no-scrollbar">
                                {activeDisplay?.images?.map((img, i) => (
                                    <button key={i} onClick={() => setMainImage(img)} className={`min-w-[80px] h-20 rounded-2xl border-2 overflow-hidden transition-all ${mainImage === img ? 'border-emerald-500 scale-105 shadow-md' : 'border-slate-100 opacity-40'}`}>
                        ...
                        <div className="p-14 space-y-10">
                            <div>
                                ...
                                <p className="text-slate-400 text-lg font-bold leading-relaxed lowercase tracking-tight">{activeDisplay?.short_description}</p>
                            </div>
                                    ...
                                    <div className="flex items-center gap-5">
                                        {mainProduct.included_products?.map((item, idx) => (
                                            <React.Fragment key={item?._id || idx}>
                                                ...
                                                >
                                                    <img src={item?.images?.[0]} className="w-14 h-14 rounded-2xl object-contain shadow-sm" alt="" />
                                                    <div className="text-left">
                                                        <div className="text-[10px] font-black text-slate-800 uppercase w-20 truncate italic">{item?.name}</div>
                                                    </div>
                                                </button>
                                                {idx === 0 && <Plus size={20} className="text-slate-300 animate-pulse" />}
                                            </React.Fragment>
                                        ))}
                                    </div>
                                </div>
                    ...
                    <div className="lg:col-span-2 space-y-12">
                        ...
                        <div className="bg-white p-14 rounded-[3.5rem] shadow-sm border border-slate-100">
                            <h2 className="text-3xl font-black mb-12 flex items-center gap-4 italic uppercase text-slate-900 tracking-tighter">
                                <ShieldCheck className="text-emerald-500" size={32} /> Key Highlights of {activeDisplay?.name}
                            </h2>
