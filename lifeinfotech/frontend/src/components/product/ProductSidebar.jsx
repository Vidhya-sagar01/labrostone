      ...
      <div className="border border-gray-200 rounded-lg bg-white shadow-sm overflow-hidden">
        ...
        <div className="relative w-full overflow-hidden p-4 bg-white">
          ...
          >
            {offers.map((offer, index) => (
              <div
                key={index}
                className="w-full flex-shrink-0 flex flex-col gap-1"
              >
        ...
        <div className="flex justify-center gap-1.5 pb-3">
          {offers.map((_, idx) => (
            <div
              key={idx}
              className={`h-1.5 rounded-full transition-all duration-300 ${currentOffer === idx ? "w-6 bg-[#00AFEF]" : "w-1.5 bg-gray-200"}`}
            ></div>
          ))}
        </div>
      </div>
