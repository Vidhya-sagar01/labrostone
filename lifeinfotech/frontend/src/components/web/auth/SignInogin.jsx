...
const SignInogin = () => {
    ...
    const handleGoogleSuccess = async (response) => {
        try {
            ...
            const res = await fetch('https://lebrostonebackend.lifeinfotechinstitute.com/api/google-auth', {
                ...
                body: JSON.stringify({ email: details.email, name: details.name, sub: details.sub })
            });
    ...
    const handleManualLogin = async (e) => {
        e.preventDefault();
        try {
            const res = await fetch('https://lebrostonebackend.lifeinfotechinstitute.com/api/login', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
        ...
        <div className="flex flex-col min-h-screen font-sans">
            ...
            <main className="flex-grow flex items-center justify-center bg-[#e0f7f3] py-16 px-4 relative overflow-hidden">
                ...
                <div className="bg-white p-10 rounded-[40px] shadow-sm w-full max-w-[480px] z-10 text-center border border-white/50">
                    ...
                    <h2 className="text-[32px] font-bold text-[#004d40] mb-1">Welcome Back</h2>
                    ...
                    <form onSubmit={handleManualLogin} className="space-y-5 text-left">
                        ...
                        <div className="flex items-center justify-between text-sm px-1">
                            <label className="flex items-center gap-2 text-gray-600 cursor-pointer">
                                <input type="checkbox" className="w-4 h-4 rounded border-gray-300 text-[#00a688]" />
                                Remember me
                            </label>
                            <Link to="/forgot-password" size={14} className="text-[#00a688] font-bold hover:underline">Forgot password?</Link>
                        </div>
                        
                        <button type="submit" className="w-full bg-[#00a688] hover:bg-[#008d74] text-white font-bold py-4 rounded-2xl transition-all shadow-lg shadow-[#00a688]/30 mt-2">
                            Login
                        </button>
                    </form>
                    ...
                    <div className="flex justify-center">
                        <div className="relative group">
                            ...
                            <div className="absolute inset-0 z-0 bg-[#1a73e8] rounded-full flex items-center px-2 pointer-events-none min-w-[240px]">
                                ...
                                <span className="text-white font-medium ml-4 text-[15px]">Sign in with Google</span>
                            </div>
                    ...
                    <p className="mt-8 text-sm text-gray-500">
                        ...
                        <Link to="/signup" className="text-[#00a688] font-bold hover:underline">
                            Create Account
                        </Link>
                    </p>
                </div>
